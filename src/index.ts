...
server.tool("hollo-world", "hollo world", {}, async () => {
    return {
        content: [
            {
                type: "text",
...
// 创建 Express 应用
const app = express();
const PORT = process.env.PORT || 8000; // 从环境变量获取端口，或使用默认值 3000
...
app.listen(PORT, () => {
    ...
    console.log(
        `Test MCP SSE Endpoint: http://localhost:${PORT}/mcp/test_mcp_sse`,
    );
    console.log(
        `Test MCP Message Endpoint: http://localhost:${PORT}/mcp/test_mcp_message`,
    );
});
